// ── VERSE COUNTS DATA LAYER ──
// Keyed by bookId (1–66), matching existing BOOK_IDS map.
// Each value is an array where index = (chapter - 1), value = verse count.
// Source: KJV canonical (SWORD project + OpenBible.info), verified by total counts.
// Validation: 66 books · 1189 chapters · 31102 verses (KJV Protestant canon)
...
Object.freeze(VERSE_COUNTS);

// ── HELPERS ──

/**
 * Returns the number of verses in a given chapter.
 * @param {number} bookId  - 1–66, matching BOOK_IDS
 * @param {number} chapter - 1-indexed
 * @returns {number|null}  - verse count, or null if out of range
 */
function getMaxVerse(bookId, chapter) {
  const book = VERSE_COUNTS[bookId];
  if (!book) return null;
  if (chapter < 1 || chapter > book.length) return null;
...
/**
 * Returns true if bookId + chapter + verse is a valid canonical reference.
 * @param {number} bookId
 * @param {number} chapter
 * @param {number} verse
 * @returns {boolean}
 */
function isValidReference(bookId, chapter, verse) {
  if (!Number.isInteger(bookId) || !Number.isInteger(chapter) || !Number.isInteger(verse)) return false;
